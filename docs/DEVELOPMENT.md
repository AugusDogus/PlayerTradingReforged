...
```sh
dotnet build src/PlayerTradingReforged/PlayerTradingReforged.csproj -c Release \
  -p:GameDir="/path/to/Valheim" \
  -p:BepInExDir="/path/to/profile/BepInEx"
```

Output: `src/PlayerTradingReforged/bin/Release/netstandard2.1/PlayerTradingReforged.dll`.
